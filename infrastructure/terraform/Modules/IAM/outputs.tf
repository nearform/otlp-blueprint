...
output "arn_role" {
  value = (var.create_ecs_role == true
    ? (length(aws_iam_role.ecs_task_execution_role) > 0 ? aws_iam_role.ecs_task_execution_role[0].arn : "")
  : (length(aws_iam_role.devops_role) > 0 ? aws_iam_role.devops_role[0].arn : ""))
}

output "name_role" {
  value = (var.create_ecs_role == true
    ? (length(aws_iam_role.ecs_task_execution_role) > 0 ? aws_iam_role.ecs_task_execution_role[0].name : "")
