locals {
  ...
  collector_image_repo_name = "${var.deployment_env}-${var.deployment_app_name}-collector-img-repo"
}
...
resource "aws_ecr_repository" "otlp-collector-image-repo" {
  name = local.collector_image_repo_name
}

resource "aws_ecr_repository_policy" "collector-image-repo-policy" {
  repository = aws_ecr_repository.otlp-collector-image-repo.name
  policy     = <<EOF
  {
    "Version": "2008-10-17",
    "Statement": [
